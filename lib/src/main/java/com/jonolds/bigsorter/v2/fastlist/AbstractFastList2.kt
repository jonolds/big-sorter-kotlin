 ...
 * @since 1.2
 */
abstract class AbstractFastList2<E>
/**
 * Sole constructor.  (For invocation by subclass constructors, typically
 * implicit.)
 */
protected constructor() : AbstractCollection<E>(), MutableList<E> {
	/**
	...
	private open inner class Itr : MutableIterator<E> {
		...
		override fun remove() {
			...
			try {
				this@AbstractFastList2.removeAt(lastRet)
	...
	private inner class ListItr(index: Int) : Itr(), MutableListIterator<E> {
		...
		override fun set(element: E) {
			...
			try {
				this@AbstractFastList2[lastRet] = element
				expectedModCount = modCount
		...
		override fun add(element: E) {
			...
			try {
				val i: Int = cursor
				this@AbstractFastList2.add(i, element)
				lastRet = -1
				cursor = i + 1
				expectedModCount = modCount
			} catch (ex: IndexOutOfBoundsException) {
...
//	}
	 ...
	 */
	internal open fun removeRange(fromIndex: Int, toIndex: Int) {
